import { modPow, modInv, mulPoly, extendSequenceFromLinearRecurrence, findRationalFunction, transformToEGF, getFactorialArray } from '../recurrence';
...
describe('modPow', () => {
  test('2^3 % mod should be 8 % mod', () => {
    expect(modPow(2n, 3n, mod)).toBe(8n % mod);
  });

  test('any number to the power of 0 should be 1', () => {
    expect(modPow(123n, 0n, mod)).toBe(1n);
  });
});

describe('modInv', () => {
  test('inverse of 2 mod 1000003 should be 500002', () => {
    expect(modInv(2n, mod)).toBe(500002n);
  });
});
...
describe('mulPoly', () => {
  test('should multiply two polynomials correctly', () => {
    const a = [1n, 2n]; // 1 + 2x
    const b = [3n, 4n]; // 3 + 4x
    const result = mulPoly(a, b, mod); // 3 + 4x + 6x + 8x^2 = 3 + 10x + 8x^2
...
describe('extendSequenceFromLinearRecurrence', () => {
  test('should extend a sequence from a linear recurrence', () => {
    ...
    const n = 5;
    const extended = extendSequenceFromLinearRecurrence(P, Q, initial_terms, n, mod);
...
describe('findRationalFunction', () => {
  ...
  test('should handle sequences starting with zeros', () => {
    const terms = [0n, 0n, 1n, 1n, 2n, 3n, 5n, 8n];
    const result = findRationalFunction(terms, mod);
...
describe('transformToEGF', () => {
  test('should transform sequence to EGF correctly', () => {
    ...
    const result = transformToEGF(terms, mod, factorial);
